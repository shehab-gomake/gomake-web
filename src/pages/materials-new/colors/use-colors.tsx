...
const useColors = () => {
  ...
  const getSheetSuppliers = useCallback(
    async (categoryName) => {
      let _data = await getAndSetColorsSuppliers(callApi, () => {}, {
  ...
  const getSheetAllWeights = useCallback(
    async (categoryName: any, supplierId) => {
      await getAndSetAllColorsData(callApi, setAllWeightsGrouped, {
        categoryName: categoryName.code,
  ...
  const getCategory = useCallback(async () => {
    const data = await getAndSetAllColorsNew(callApi, setSheetCategories);
    if (!categoryName) {
  ...
  const onClickAddSupplier = async () => {
    await callApi("POST", `/v1/colors/add-supplier-catogry`, {
      categoryName: selectedMaterials.code,
      supplierId: sheetStore.selectedSupplier,
    });
  ...
  const onChangeSupplierToDefault = async (option, value) => {
    if (value) {
      await callApi("POST", `/v1/colors/update-default-supplier`, {
        categoryName: selectedMaterials.code,
