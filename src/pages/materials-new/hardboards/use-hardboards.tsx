...
const useHardboards = () => {
    ...
    if (isChecked) {
      ...
      setSelectedItems((prevSelectedItems) =>
        prevSelectedItems.filter(
          (item) =>
            !(item.sizeId === sizeId && item.thicknessId === thicknessId)
        )
      );
    }
  };
  ...
  const getSheetSuppliers = useCallback(
    async (categoryName) => {
      let _data = await getAndSetHardboardsSuppliers(callApi, () => {}, {
  ...
  const getSheetAllWeights = useCallback(
    async (categoryName: any, supplierId) => {
      setIsLoader(true);
      await getAndSetAllHardboardsSizes(callApi, setAllWeightsGrouped, {
        categoryName: categoryName?.key,
  ...
  const getCategory = useCallback(async () => {
    const data = await getAndSetHardboardsCategory(callApi, setSheetCategories);
    if (!categoryName) {
  ...
  const onClickAddSupplier = async () => {
    await callApi("POST", `/v1/hardboards/add-supplier-catogry`, {
      categoryName: selectedMaterials?.key,
      supplierId: sheetStore.selectedSupplier,
    });
  ...
  const onChangeSupplierToDefault = async (option, value) => {
    if (value) {
      await callApi("POST", `/v1/hardboards/update-default-supplier`, {
        categoryName: selectedMaterials?.key,
