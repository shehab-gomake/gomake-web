...
const useTubes = () => {
  ...
  const updatePricePetTon = useCallback(async () => {
    const res = await callApi("POST", `/v1/glues/size-id-settngs`, {
  ...
  const getSheetSuppliers = useCallback(
    async (categoryName) => {
      let _data = await getAndSetGluesSuppliers(callApi, () => {}, {
  ...
  const getCategory = useCallback(async () => {
    const data = await getAndSetAllGlue(callApi, setSheetCategories);
    if (!categoryName) {
  ...
  const onChangeSupplierToDefault = async (option, value) => {
    if (value) {
      await callApi("POST", `/v1/glues/update-default-supplier`, {
        categoryName: selectedMaterials.code,
