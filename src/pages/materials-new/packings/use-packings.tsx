...
const usePackings = () => {
  ...
  const handleCheckboxChange = (volumeId, isChecked) => {
    if (isChecked) {
      ...
      setSelectedItems((prevSelectedItems) =>
        prevSelectedItems.filter((item) => !(item.volumeId === volumeId))
      );
    }
  };
  useEffect(() => {
  ...
  const getCategory = useCallback(async () => {
    const data = await getAndSetPackingsCategory(callApi, setSheetCategories);
    if (!categoryName) {
