...
const useFrames = () => {
  ...
  const updatePricePetTon = useCallback(async () => {
    const res = await callApi("POST", `/v1/frames/size-id-settngs`, {
  ...
  const getSheetSuppliers = useCallback(
    async (categoryName = false) => {
      let _data = await getAndSetFramesSuppliers(callApi, () => {}, {
        categoryName,
      });
      _data.push({
        name: "Add new",
  ...
  const getSheetAllWeights = useCallback(
    async (categoryName: string, supplierId) => {
      await getAndSetAllFramesSizes(callApi, setAllWeightsGrouped, {
        categoryName,
  ...
  const getCategory = useCallback(async () => {
    const data = await getAndSetFramesCategory(callApi, setSheetCategories);
    if (!categoryName) {
  ...
  const onClickAddSupplier = async () => {
    await callApi("POST", `/v1/frames/add-supplier-catogry`, {
      categoryName: selectedMaterials,
      supplierId: sheetStore.selectedSupplier,
    });
  ...
  const onChangeSupplierToDefault = async (option, value) => {
    if (value) {
      await callApi("POST", `/v1/frames/update-default-supplier`, {
        categoryName: selectedMaterials,
