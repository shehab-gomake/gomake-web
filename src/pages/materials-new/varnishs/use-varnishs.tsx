...
const useVarnishs = () => {
  ...
  const updatePricePetTon = useCallback(async () => {
    const res = await callApi("POST", `/v1/varnishs/size-id-settngs`, {
  ...
  const getSheetSuppliers = useCallback(
    async (categoryName) => {
      let _data = await getAndSetVarnishsSuppliers(callApi, () => {}, {
  ...
  const getCategory = useCallback(async () => {
    const data = await getAndSetAllVarnishsNew(callApi, setSheetCategories);
    if (!categoryName) {
  ...
  const onClickAddSupplier = async () => {
    await callApi("POST", `/v1/varnishs/add-supplier-catogry`, {
      categoryName: selectedMaterials.code,
      supplierId: sheetStore.selectedSupplier,
    });
  ...
  const onChangeSupplierToDefault = async (option, value) => {
    if (value) {
      await callApi("POST", `/v1/varnishs/update-default-supplier`, {
        categoryName: selectedMaterials.code,
