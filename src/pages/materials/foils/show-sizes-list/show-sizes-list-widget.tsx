...
const ShowSizesListWidgetForFoils = ({ item: _item }: any) => {
  const {
    headerTable,
    state,
    suppliers,
          ...
          tableRows={supplierData?.map((item: any) => {
            ...
            return {
              ...
              pricePerRoll: (
                <GomakeTextInput
                  type="number"
                  placeholder={t("materials.foils.pricePerRoll")}
                  style={clasess.textInputStyle}
                  value={
                    state[`pricePerRoll-${supplierId}`] || item.pricePerRoll
                  }
                  onChange={(e: any) =>
                    onChangeState("pricePerRoll", supplierId, e.target.value)
                  }
                />
              ),
              pricePerSquareMeter: (
                <GomakeTextInput
                  type="number"
                  placeholder={t("materials.foils.pricePerSquareMeter")}
                  style={clasess.textInputStyle}
                  value={
              ...
              currency: suppliersCurrencies?.length > 0 && (
                <GoMakeAutoComplate
                  ...
                  style={clasess.dropDownListContainer}
                  placeholder={t("materials.foils.selectCurrency")}
                  value={
...
export { ShowSizesListWidgetForFoils };
