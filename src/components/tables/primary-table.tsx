          ...
          <PrimaryTableRow>
            {headers?.map((header, index) => {
              if (index === 0 && stickyHeader) {
                return (
                  <PrimaryTableCell style={classes.stickyHeader}>
                    {header}
                  </PrimaryTableCell>
                );
              } else {
                return (
                  <PrimaryTableCell align={"center"}>{header}</PrimaryTableCell>
                );
              }
            })}
          </PrimaryTableRow>
            ...
            <PrimaryTableRow>
              {row.map((cell, index) => {
                if (index === 0 && stickyFirstCol) {
                  return (
                    <PrimaryTableCell style={classes.sticky}>
                      {cell}
                    </PrimaryTableCell>
                  );
                } else {
                  return (
                    <PrimaryTableCell align={"center"}>{cell}</PrimaryTableCell>
                  );
                }
              })}
            </PrimaryTableRow>
