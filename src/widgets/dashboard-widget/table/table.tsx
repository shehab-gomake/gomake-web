    ...
    return (
        boardsMissions.length > 0 ? <div>
            <div style={classes.tableContainer}>
                <table className={'table'} style={classes.fixedTable}>
                    <thead>
                    <tr>
                        <th style={{...classes.tableHead, ...classes.topLeftCell}}>#</th>
                    ...
                    <tbody>
                    {
                        boardsMissions && boardsMissions.map((board: IBoardMissions, index: number) => {
                            return (
                                <tr key={index}>
                                    <td style={{...classes.fixedTableCell, ...classes.firstColumn}}>{index + 1}</td>
                                    <td style={classes.fixedTableCell}>{board.isUrgent ?
                                        <ElectricBoltSharpIcon color={"error"}/> : ''}</td>
                                    <td style={classes.fixedTableCell}>{board.code}</td>
                ...
                <div style={classes.scrollTableWrapper}>
                    <table style={classes.scrollableTable}>
                        <thead>
                        <tr>
                            {
                                usedMachines.map((machine: IMachine, index: number) => {
                                    return (
                                        <th key={machine.id}
                                            style={index + 1 === usedMachines.length  ? {...classes.tableHead, ...classes.topRightCell} : classes.tableHead}>
                                            <div style={classes.machineName}>
