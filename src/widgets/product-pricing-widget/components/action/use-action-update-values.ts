...
const useActionUpdateValues = () => {
  ...
  const updateSelectedWorkFlow = (actions: IWorkFlowAction[]) => {
    const totalPrice = actions.reduce(
      (sum, action) =>
    ...
    const deliveryTime = actions.reduce(
      (sum, action) =>
        action.source === EWorkSource.OUT
          ? action.totalProductionTime?.outSourceValues &&
            action.totalProductionTime?.outSourceValues[0]
            ? sum + +action.totalProductionTime?.outSourceValues[0]
            : sum
          : sum + +action.totalProductionTime.values[0],
      0
    );
