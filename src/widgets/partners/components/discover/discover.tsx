        ...
        <div style={classes.mainContainer} >
            <Stack direction={'column'} gap={"10px"} padding={"10px"}>
                ...
                <Stack direction={'row'} gap={"16px"} width={"100%"} alignItems={"end"} >
                    <SearchInputComponent onChange={null} placeHolder="Search for company, product, machine, etc.." />
                    <Stack direction={'row'} gap={"16px"}>
                    {
                        discoverInputs(state).map(item => <FormInput input={item as IInput} changeState={null} error={false} readonly={false} />)
                    }
                    </Stack>
