...
const useRule = () => {
    ...
    const deleteRule = useCallback(async (actionId: string,propertyId: string,ruleType: number, id: string) => {
        const res = await callApi("PUT", `/v1/printhouse-config/print-house-action/delete-rule/${actionId}/${propertyId}/${ruleType}/${id}`, {
            actionId,
        });
        if (res?.success) {
