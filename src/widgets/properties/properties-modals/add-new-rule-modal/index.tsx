...
const AddNewRuleModal = ({ openModal, onClose, actionId, propertyId, ruleType }) => {
  ...
  const [fields, setFields] = useState([{ linkCondition: '', category: '', firstPart: '', condition: '', secondPart: '' }])
  const [fieldsStates, setFieldsStates] = useState([{ firstPart: [], secondPart: [] }])
  const [firstPartDefultValue, setFirstPartDefultValue] = useState([{ defultValue: null, isDisabled: false }])
  const [secondPartTypeState, setSecondPartTypeState] = useState([{ isInputField: false }])
  ...
  const [expression, setExpression] = useState('Your rules will viewed here . . .');
  ...
  const handleFormChange = (index, event, fieldNameId, value) => {
    ...
    if (fieldNameId == 1) {
      ...
      if (fields[index].category === "Machine" || fields[index].category === "Machine Category" || fields[index].category === "Client"
        || fields[index].category === "Client Type" || fields[index].category === "Material" || fields[index].category === "Material Category") {
      ...
      data[index].secondPart = value?.name == undefined ? value?.label : value?.name;
    }
    setFields(data);
  }
  ...
  const addFields = () => {
    let newfield = { linkCondition: '', category: '', firstPart: '', condition: '', secondPart: '' }
  ...
  const removeFields = (index) => {
    ...
    if (data.length == 0) {
      data.push({ linkCondition: '', category: '', firstPart: '', condition: '', secondPart: '' })
  ...
  const create = async () => {
    ...
    if (values?.length != 0) {
      ...
      let val = '(';
      values?.map((m, index) => {
        val += (' ' + m.linkCondition + ' ' + m.firstPart + ' ' + m.condition + ' ' + m.secondPart + ' ')
        ruleName = m.category
      })
      val += ')';
  ...
  const validateForm = (): any => {
    let values = []
    fields.map((m, index) => {
      if (m.firstPart != '' && m.condition != '' && m.secondPart != ''
        && ((m.linkCondition == '' && index == 0) || (m.linkCondition != '' && index != 0)) && value != '') {
        values.push(m);
      }
    })
    if (values?.length == 0) {
  ...
  return (
    <>
      <GoMakeModal
        ...
        onClose={() => { onClose(), setFields([{ linkCondition: '', category: '', firstPart: '', condition: '', secondPart: '' }]) }}
      ...
      >
          ...
          <form >
            {fields.map((field, index) => {
              return (
                <div key={index}>
                  {index != 0 &&
                    <div style={{ display: "block", marginTop: 30, marginBottom: 30 }}>
                      <label style={clasess.inputLable}>{t("properties.condtionBetween")}</label>
                      <GoMakeAutoComplate
                        options={mainconditions}
                        style={clasess.dropDownListContainer}
                        placeholder={t("properties.condtion")}
                        value={field.linkCondition}
                        onChange={(event, value) => { handleFormChange(index, event, 1, value) }}
                      />
                    </div>
                  }
                  ...
                  <div style={clasess.inputsContainer}>
                    ...
                    <div>
                      <label style={clasess.inputLable}>{t("properties.category")}</label>
                      <GoMakeAutoComplate
                        options={categories}
                        style={clasess.dropDownListContainer}
                        placeholder={t("properties.category")}
                        value={field.category}
                        onChange={(event, value) => { handleFormChange(index, event, 2, value) }}
                      />

                    </div>
                    <div>
                      <label style={clasess.inputLable}>{t("properties.statment")}</label>
                      <GoMakeAutoComplate
                        ...
                        onChange={(event, value) => { handleFormChange(index, event, 3, value) }}
                      />

                    </div>
                    <div>
                      <label style={clasess.inputLable}>{t("properties.condtion")}</label>
                      <GoMakeAutoComplate
                        options={conditions}
                        style={clasess.dropDownListContainer}
                        placeholder={t("properties.condtion")}
                        value={field.condition}
                        onChange={(event, value) => { handleFormChange(index, event, 4, value) }}
                      />
                    </div>
                    <div>
                      <label style={clasess.inputLable}>{t("properties.statment")}</label>
                      ...
                      {secondPartTypeState[index].isInputField &&
                        <GomakeTextInput
                          ...
                          onChange={(event, value) => { handleNumberField(index, event, 5, value) }}
                        />
                      }
                      {!secondPartTypeState[index].isInputField &&
                        <GoMakeAutoComplate
                          ...
                          placeholder={t("properties.statment")}
                          value={field.secondPart}
                          onChange={(event, value) => { handleFormChange(index, event, 5, value) }}
                        />
                      }
                    </div>
                    <div style={{ display: "flex", alignItems: "center", cursor: "pointer" }} onClick={removeFields}>
                      <DeleteMenuIcon />
                    </div>
                  </div>
                </div>
        ...
        <div style={clasess.AddNewRuleDiv} >
          <AddPlusIcon /> <span onClick={addFields} style={clasess.spanAddNewRule}>{t("properties.addNewRule")}</span>
        </div>
        <div style={clasess.valueContainer}>
          ...
          <GomakeTextInput
            ...
            placeholder="value"
            value={value}
            onChange={(event, value) => { handleNumberField(0, event, 6, value) }}
          />
        </div>
        <div style={clasess.btnContainer}>
          <GomakePrimaryButton style={clasess.sendBtn} onClick={() => { create() }}>
            {t("properties.create")}
          </GomakePrimaryButton>
        </div>
        <div>
          <div >
            <label style={{ ...FONT_FAMILY.Lexend(500, 12), color: primaryColor(500), fontSize: 16, marginBottom: 10 }}>Terminal</label>
            <textarea disabled={true} style={clasess.textarea}
