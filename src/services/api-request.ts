...
const apiRequest = async (method = "GET", url: string, data: any = {}, secondServer: boolean = false) => {
  try {
    ...
    const SERVER2 = 'https://production-service-dev.gomake.co.il';
...
export const gomakeClientAPIRequest = async (method = "GET", url: string, data: any = {}, secondServer: boolean = false) => {
  try {
    const reqUrl = "/PrintHouseProfile/GetPrintHouseId"
    const options: any = {
      method,
      url: reqUrl,
      data,
